...
float TP700TemperatureModel::getTemperatur(QString ch)
{
	if (ch.length() < 3)return -99.99;
	uint8_t index = ch.mid(2).toUInt()&0xff;
	if (index < 0 || index>32)return -99.99;
	return vData[index].fval;
}
...
void TP700TemperatureModel::on_receive(QVector<uint16_t> data)
{
	
	{
		...
		else if (m_register_ == 8)
		{
			...
			{
				//处理负数情况，高位符号位，1是负数，0是正数
				if (data.at(i) & 0x8000)
				{
					vData[i].fval = -((data.at(i) & (~0x8000)) / 10.0);
				}
				else
				{
...
void TP700TemperatureModel::on_pbStart_clicked(bool b)
{
	if (b)
	{
		...
		m_debug = this->ui.tableWidget->item(4, 1)->text().toInt();
		if (cb)
