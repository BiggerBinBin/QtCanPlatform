...
import tkinter.tix                     # TK extensions library
import tkinter.messagebox            # Simple-Messages library
import traceback               # Error-Tracing library
import string                  # String functions
import tkinter.font                  # Font-Management library
import tkinter.simpledialog          # Dialog library
...
class CFrameDialog(tkinter.simpledialog.Dialog):
    ...
    def __init__(self, parent, pGFT):
        ...
        tkinter.simpledialog.Dialog.__init__(self, parent, "Global Frames Table")
    ...
    def body(self, master):
        ...
        self.m_ListFont = tkinter.font.Font(family="Lucida Console", size="10")
        ...
        self.cbbCST = tkinter.tix.ComboBox(
        ...
        for value in self.m_linCSTs:
            self.cbbCST.insert(
                tkinter.tix.END, CFrameDefinition.getFrameCSTAsString(value))
        # property : frame direction
        Label(propertyBox, anchor=W, text="Direction:").grid(
            row=2, column=3, sticky=W)
        self.cbbDirection = tkinter.tix.ComboBox(
        ...
        for value in self.m_linDirections:
            self.cbbDirection.insert(
                tkinter.tix.END, CFrameDefinition.getFrameDirectionAsString(value))
        # property : length
        Label(propertyBox, anchor=W, text="Length:").grid(
            row=3, column=3, sticky=W)
        self.cbbLength = tkinter.tix.ComboBox(
        ...
        for value in range(0, 9):
            self.cbbLength.insert(tkinter.tix.END, str(value))
...
class PLinApiExample:
    ...
    def __init__(self, parent):
        ...
        # function fails with python 3.3
...
# *****************************************************************
    ...
    def initializeConnectionWidgets(self):
        ...
        self.cbbChannel = tkinter.tix.ComboBox(
        ...
        for name, value in self.m_dataHws.items():
            self.cbbChannel.insert(tkinter.tix.END, name)
        ...
        self.cbbBaudrates = tkinter.tix.ComboBox(self.gbConnection)
        ...
        for name, value in self.m_dataHwModes.items():
            self.cbbHwMode.insert(tkinter.tix.END, name)
    ...
    def initializeReadingWidgets(self):
        ...
        # message listbox
        self.lstMessages = tkinter.tix.TList(
    ...
    def initializeWritingWidgets(self):
        ...
        self.cbbID = tkinter.tix.ComboBox(
    ...
    def updateFrameIds(self):
        ...
        for pFrame in self.m_pGFT.m_lFrameDefinitions:
            ...
            self.cbbID.insert(tkinter.tix.END, strId)
            if (strId == strIdSel):
                bIdSelected = True
    ...
    def showMessageError(self, linError):
        ...
        Returns:
            None
        """
        tkinter.messagebox.showinfo("Error!", self.getFormattedError(linError))
        ...
        if (lwHw.value != 0):
            ...
            if (str(lnCurrBaud.value) in self.m_dataHwBaudrates):
                self.cbbBaudrates.pick(
                    list(self.m_dataHwBaudrates.keys()).index(str(lnCurrBaud.value)))
    ...
    def refreshHardware(self):
        ...
        if (linResult == PLinApi.TLIN_ERROR_OK):
            ...
            if (lwCount.value == 0):
                ...
                self.cbbChannel.insert(tkinter.tix.END, strHw)
