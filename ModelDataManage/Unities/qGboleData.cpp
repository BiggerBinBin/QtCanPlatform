...
void qGboleData::save()
{
	...
	for (int i = 0; i < pGboleData.size(); i++)
	{
		...
		mItem.insert("isLIN", pGboleData.at(i).isLIN);
		//下面是自动化测试用的参数
		...
		for (int j = 0; j < pGboleData.at(i).cItem.size(); j++)
		{
			...
			{
				QJsonArray pItem;
				pGboleData.at(i).cItem.at(j).pItem.at(k).startByte = pGboleData.at(i).cItem.at(j).pItem.at(k).startBit / 8;
...
void qGboleData::read()
{
	...
	for (int i = 0; i < keyList.size(); i++)
	{
		...
		ptem.isLIN = rootSecond["isLIN"].toBool(0);
